...
def readCSV():
	names = ['atmcond', 'travspd', 'travlimit', 'injury/class']
	dataset = pandas.read_csv('3dataSet(all).csv', names = names)
	return dataset
...
def main():
	...
	with open('3dataSet(all).txt', 'a') as f:
		for i in res:
			print('', i, file=f)
...
def machineLearning(dataset):
	...
	for i in range(1,8):
		models.append(("pol_SVM_{}".format(i), SVC(kernel='poly', degree = i)))
	for i in range(1,20):
